...
export default function SimulationPage() {
  ...
  const [metricsPanelSize, setMetricsPanelSize] = useState(() => {
    try {
      ...
      if (saved) {
        ...
        // Guard against NaN and clamp to valid bounds (20-35%) - narrow panel for side-by-side canvases
        if (!Number.isFinite(value)) {
          localStorage.setItem('sfd-metrics-panel-size', '20');
          return 20;
        }
        // Enforce 20% min, 35% max so canvases display side-by-side
        const clamped = Math.max(20, Math.min(35, value));
  ...
  }); // Default 20% for metrics panel, narrow to keep canvases side-by-side
  ...
  const [selectedRegimeKey, setSelectedRegimeKey] = useState<string | null>(null); // Start with Default highlighted
        ...
        >
          <div className="bg-neutral-900/90 backdrop-blur-xl rounded-2xl border border-white/15 shadow-lg">
            <div className="flex items-center justify-around h-20 px-2">
                  ...
                  {/* Run button - with attention effects until pressed */}
                  <div className="relative">
                    ...
                    <button
                      onClick={() => {
                        // On first press, start the simulation
                        if (showRunPulse && !state.isRunning) {
                          handlePlay();
                        }
                      ...
                      style={{
                        animation: showRunPulse ? 'breathe 2.5s ease-in-out infinite' : undefined,
                      }}
  ...
  return (
    <div className="flex flex-col h-screen bg-background">
      ...
      <ResizablePanelGroup direction="horizontal" className="flex-1 overflow-hidden" onLayout={(sizes) => {
        ...
        <ResizablePanel defaultSize={100 - metricsPanelSize} minSize={65} maxSize={80} className="relative bg-gray-950 min-w-0 flex flex-col">
        ...
        <ResizablePanel defaultSize={metricsPanelSize} minSize={20} maxSize={35} className="border-l border-border bg-card flex flex-col overflow-hidden">
          {showPerturbControls && (
